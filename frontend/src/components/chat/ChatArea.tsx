...
import { useStreamClient } from "@/api/streamClient";
...
export const ChatArea = ({ onToggleSources }: ChatAreaProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error("Auth/Stream failed:", error);
      updateLastMessage(
        activeChatId,
        " [Error: Identity Verification Failed or Backend Offline]",
      );
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-full relative overflow-hidden">
      ...
      {/* Chat Messages - Scrollbar is set to 'no-scrollbar' for blending */}
      <div
        ...
        {activeChat.messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {/* View Trace Button (The Identity Bridge visualization)  */}
              {msg.role === "assistant" && msg.content && (
                ...
                >
                  ...
                  <span className="text-teal-300 opacity-50 group-hover:opacity-100 transition-opacity">
                    • {msg.traceData?.match || "98%"} Secure Match
                  </span>
                </button>
              )}
            </div>
        ...
        <div className="max-w-3xl mx-auto relative group">
          <div className="relative flex items-center bg-white/40 border border-slate-200/60 rounded-2xl transition-all focus-within:border-teal-500/40 focus-within:ring-8 focus-within:ring-teal-500/5 shadow-sm backdrop-blur-xl">
            ...
            <textarea
              ...
              placeholder="Ask Intellectus about Dexter Corp..."
          ...
          <p className="text-[10px] text-center text-slate-400 mt-3 tracking-widest font-bold uppercase">
            Intellectus AI v1.0 • Multi-Tenant Architecture
          </p>
        </div>
      </div>
    </div>
...
const EmptyState = () => (
  <div className="flex-1 flex flex-col items-center justify-center bg-slate-50/20 p-8">
    ...
    <h2 className="text-sm font-bold text-slate-800 uppercase tracking-widest mb-2">
      Intellectus Node Ready
    </h2>
    <p className="text-xs text-slate-400 font-medium tracking-tight">
      Select a workspace or initialize a new neural query.
    </p>
  </div>
);
